...
public class Contenedor<T extends Comparable<T>> implements Pila<T>, Cola<T> {
	...
	public int[] buscarTodos(Object e) {
		...
		T[] nuevaTabla = Arrays.copyOf(objetos, objetos.length); //Creamos nueva tabla para no tocar la original
		
		for (int i = 0; i < nuevaTabla.length; i++) { 
			if (nuevaTabla[i].equals(e)) {  //Si el elemento es igual que nuestro objeto
				indices = Arrays.copyOf(indices, indices.length + 1); //Ampliamos
				indices[indices.length - 1] = i; //Copiamos el índice
			}
		}
	...
	public boolean eliminarTodos(Object e) {
		...
		T[] tablaAux; //Creamos la auxiliar
		
		for (int i = 0; i < objetos.length; i++) { 
			if (objetos[i].equals(e)) {  //Si el elemento es igual que nuestro objeto
				tablaAux = Arrays.copyOf(objetos, objetos.length - 1); //Reducimos la tabla
				System.arraycopy(objetos, i + 1, tablaAux, i, tablaAux.length - i); //Volcamos los demás datos
				
				objetos = tablaAux; //Copiamos
				alterada = true;
			}
		}
